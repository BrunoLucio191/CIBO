...
def draw_word(word,sz,emphasized=False):
    ...
    ImageDraw.Draw(tl).text((pad,pad),word,font=ft,fill=ORANGE if emphasized else (255,255,255,255))
...
def build_caption(words,flags):
    ...
    L=[]
    for ln in lines:
        ws=[(w,s,fl)+draw_word(w,s,fl) for w,s,fl in ln]
        L.append(ws)
